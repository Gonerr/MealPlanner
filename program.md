...
Например, вот так делать нельзя: GET /delete-account

или: GET /transfer?to=hacker&amount=10000
...
## Правило: GET должен быть безопасным методом: он не должен менять состояние на сервере. Для изменений используют POST, PUT, PATCH, DELETE, но сам по себе POST тоже не защищает от CSRF. Нужны дополнительные меры.
...
Например: Set-Cookie: sessionId=abc123; HttpOnly; Secure; SameSite=Lax

Основные варианты:

- SameSite=Strict - cookie почти не отправляется в cross-site сценариях

- SameSite=Lax - cookie отправляется в более безопасных сценариях, например при обычном переходе по ссылке, но не в большинстве cross-site POST-запросов

- SameSite=None - cookie можно отправлять cross-site, но обязательно нужно Secure
...
При небезопасных методах сервер может проверять заголовки: Origin: https://app.example.com
или:
Referer: https://app.example.com/settings

Если запрос пришёл с чужого сайта: Origin: https://evil.example.com сервер его отклоняет.
...
Плохо: GET /api/orders/42/delete
Лучше: DELETE /api/orders/42 или POST /api/orders/42/cancel
...
Например, перед сменой email, удалением аккаунта или переводом денег. Это не заменяет CSRF-токены, но помогает защитить самые опасные операции.

## А CORS защищает от CSRF?

Вот тут важный момент: CORS сам по себе не является защитой от CSRF. CORS ограничивает, может ли JavaScript на чужом сайте прочитать ответ. Но CSRF часто не требует чтения ответа. Злоумышленнику достаточно отправить запрос.
...
Даже если браузер не отдаст ответ вредному сайту, действие на сервере уже могло выполниться. Поэтому нельзя говорить: “У нас CORS настроен, значит CSRF не страшен”. Не-а, это разные механизмы.

_CSRF — это атака, при которой злоумышленник заставляет браузер авторизованного пользователя отправить запрос на доверенный сайт. Если авторизация основана на cookies, браузер автоматически приложит их к запросу, и сервер может принять действие как выполненное пользователем. CSRF опасен для операций, меняющих состояние: смена email, перевод денег, удаление данных, изменение настроек. Для защиты используют CSRF-токены, которые сервер выдаёт странице и проверяет при запросе, атрибут SameSite у cookies, проверку Origin или Referer, запрет изменения данных через GET, а для критичных действий — повторное подтверждение. CORS не является полноценной защитой от CSRF, потому что атакующему часто не нужно читать ответ, достаточно отправить запрос._

# Вопрос 9. Вопрос 9: как работает HTTP-кэширование, что такое Cache-Control, ETag, Last-Modified и разница между свежестью и условным запросом.

- HTTP-кэширование нужно, чтобы браузер не скачивал одни и те же ресурсы снова и снова.

Например, у сайта есть: style.css, app.js, logo.png

Если каждый раз при открытии страницы браузер будет заново скачивать все CSS, JS, картинки и шрифты — сайт будет медленнее, сервер будет грустнее, пользователю будет больнее. Кэширование позволяет сохранить ответ и использовать его повторно.

#### Кэшироваться могут разные ответы:
...
Но не всё стоит кэшировать одинаково.

Например:

- logo.png → можно кэшировать долго
- app.8f3a1.js → можно кэшировать очень долго
- /profile → осторожно, данные пользователя
- /api/orders/status → зависит от свежести данных

Главное: сервер через заголовки говорит браузеру, можно ли хранить ответ и как долго считать его актуальным.

### Cache-Control — главный заголовок для управления кэшем.

Пример: Cache-Control: max-age = 3600
Это значит: Ответ можно считать свежим 3600 секунд, то есть 1 час.

Если браузер снова запросит этот ресурс в течение часа, он может взять его из кэша без обращения к серверу.

#### max-age

Cache-Control: max-age=60
Ответ свежий 60 секунд.

Если пользователь обновит страницу через 30 секунд, браузер может использовать кэш. Если через 2 минуты — ресурс уже устарел, и браузеру нужно проверить его у сервера.

#### no-store

Cache-Control: no-store
Это самый строгий вариант: не хранить ответ вообще.
...
То есть браузер не должен сохранять такой ответ в кэше.

#### no-cache

Вот тут название коварное, потому что no-cache не означает «не кэшировать вообще».

Cache-Control: no-cache

Означает: Можно сохранить в кэше, но перед повторным использованием нужно спросить сервер, актуален ли ответ. То есть браузер не может просто молча взять старую версию. Он должен сделать условный запрос.

Очень коротко:

- no-store → вообще не сохранять
- no-cache → сохранять можно, но перед использованием перепроверять

Да, название no-cache максимально rude, потому что звучит не так, как работает.

### private и public

#### Cache-Control: private

Ответ можно хранить только в личном кэше пользователя, например в браузере. Общие прокси и CDN не должны его кэшировать.

Подходит для персональных данных:
...
Ответ можно хранить и в браузере, и в промежуточных кэшах: CDN, proxy.Подходит для публичных ресурсов:
...
### Freshness: свежий ответ

- Свежесть означает: браузер считает кэшированную копию актуальной и может использовать её без запроса к серверу.

Например, сервер отдал: Cache-Control: max-age=3600
Браузер сохранил файл style.css.

Через 10 минут пользователь снова открывает страницу. Браузер такой: max-age ещё не истёк, файл свежий, беру из кэша. Сервер вообще не трогается.

Схема:

- Первый раз:
  браузер → сервер → получил файл → положил в кэш

- Пока файл свежий:
  браузер → берёт из кэша

Это самый быстрый вариант, потому что нет сетевого запроса.

Когда кэш устарел. Допустим, max-age=3600, но прошёл уже час. Теперь браузер не уверен, что файл актуален. У него есть два варианта:

- скачать ресурс заново
  или
- спросить сервер: моя версия ещё актуальна?

Вот тут появляются ETag и Last-Modified.

### ETag

- ETag — это идентификатор версии ресурса.

Например, сервер отдаёт файл:

```html
HTTP/1.1 200 OK Cache-Control: no-cache ETag: "abc123"
```

Браузер сохраняет тело ответа и ETag В следующий раз браузер может спросить:

GET /style.css
If-None-Match: "abc123"

Смысл: Сервер, у меня есть версия abc123. Она ещё актуальна?

1. Если файл не изменился, сервер отвечает:

   HTTP/1.1 304 Not Modified

   Тела ответа нет. Браузер использует свою старую копию из кэша.

2. Если файл изменился, сервер отвечает:
   ...
   То есть ETag позволяет не скачивать файл заново, если он не поменялся.
...
```html
HTTP/1.1 200 OK Last-Modified: Wed, 09 Jul 2026 10:00:00 GMT
```

Браузер потом отправляет:

```html
If-Modified-Since: Wed, 09 Jul 2026 10:00:00 GMT
```

Сервер, ресурс менялся после этого времени?
...
2. Если менялся:
   200 OK
   новая версия ресурса

### ETag vs Last-Modified

Они решают похожую задачу: помогают проверить, изменилась ли кэшированная версия. Но ETag обычно точнее.

- Last-Modified — проверка по времени изменения
- ETag — проверка по идентификатору версии/содержимого

#### Почему Last-Modified может быть менее точным?

Например, ресурс изменился два раза в пределах одной секунды, а точность времени недостаточная. Или время на сервере настроено странно. ETag может точнее отражать именно версию содержимого. На практике сервер может использовать оба.

### Условный запрос

- Условный запрос — это когда браузер спрашивает сервер: «отдай ресурс только если он изменился».

Примеры условных заголовков:
If-None-Match: "abc123"
или:
If-Modified-Since: Wed, 09 Jul 2026 10:00:00 GMT
...
Это не ошибка. Это нормальный ответ: “Ничего не поменялось, используй свою кэшированную копию.”
Плюс: тело ответа не передаётся, экономится трафик.

Но сетевой запрос всё равно был. Поэтому это медленнее, чем полностью свежий кэш, но быстрее, чем скачивать файл заново.

### Разница между свежестью и условным запросом

- Свежий кэш:
  - Браузер вообще не обращается к серверу.
  - Просто берёт ресурс из кэша.
    Например: Cache-Control: max-age=3600. Пока час не прошёл — ресурс fresh.
- Условный запрос:
  - Браузер обращается к серверу и спрашивает, изменилась ли версия.
  - Если нет — получает 304 и использует кэш.

То есть:

- freshness → без запроса к серверу
- conditional GET → запрос к серверу есть, но тело можно не скачивать

Свежий кэш:
браузер → кэш → готово

Условный запрос:
браузер → сервер: изменилась версия?
сервер → 304 Not Modified
браузер → берёт тело из кэша
Кэширование статики с хешами

В реальных фронтенд-приложениях часто файлы называются так:

- app.8f3a1c.js
- styles.a91bd.css

Хеш в имени меняется, когда меняется содержимое файла. Тогда можно ставить очень длинный кэш:

- Cache-Control: public, max-age=31536000, immutable

Потому что если файл изменится, у него будет новое имя: app.oldhash.js → app.newhash.js. Старый файл можно безопасно хранить долго.

А вот HTML обычно кэшируют осторожнее, потому что именно HTML указывает, какие JS/CSS-файлы загружать.
...
- корзину;
- админские страницы;
- результаты, которые должны быть строго свежими.

Для таких ответов можно использовать: Cache-Control: no-store или более аккуратные правила, если данные можно перепроверять.

_HTTP-кэширование позволяет браузеру или промежуточным кэшам повторно использовать ранее полученные ответы. Заголовок Cache-Control определяет правила кэширования: max-age задаёт время свежести, no-store запрещает хранение, no-cache разрешает хранить ответ, но требует перепроверки перед использованием. ETag — идентификатор версии ресурса, а Last-Modified — дата последнего изменения. Когда кэш устарел, браузер может отправить условный запрос с If-None-Match или If-Modified-Since. Если ресурс не изменился, сервер вернёт 304 Not Modified, и браузер использует локальную копию. Свежий кэш используется без обращения к серверу, а условный запрос требует обращения к серверу, но может не скачивать тело ответа повторно._

Мини-проверка:

Что означает ответ сервера:

HTTP/1.1 304 Not Modified

Браузеру нужно скачать ресурс заново или можно использовать кэшированную копию?

# вопрос 10 — обмен данными в реальном времени: WebSocket и SSE.

#### Что значит «обмен в реальном времени»

Обычный HTTP работает так:
...
Клиент сам спрашивает сервер: «есть новые сообщения?» Сервер отвечает.

_Но для чата, уведомлений, онлайн-статусов, биржевых котировок, трекинга доставки и live-логов это неудобно: данные могут появиться в любой момент, и хочется, чтобы сервер сам быстро сообщил клиенту._ Есть несколько способов:

- Polling — клиент постоянно спрашивает сервер
- Long polling — клиент спрашивает, сервер держит запрос открытым
- SSE — сервер постоянно отправляет события клиенту
- WebSocket — постоянное двустороннее соединение
- Polling

Самый простой вариант — клиент раз в несколько секунд делает запрос:
...
- сервер нагружается даже когда новых данных нет.

Если новые уведомления появляются раз в час, а клиент спрашивает каждые 5 секунд — это прям ходить к холодильнику каждые 5 секунд и удивляться, что там всё ещё ничего нового.

## Long polling

Long polling чуть умнее. Клиент отправляет запрос: Есть новые сообщения?
Сервер не отвечает сразу, а держит соединение открытым, пока не появятся данные или не истечёт таймаут.

Схема:

1. клиент → запрос
2. сервер ждёт...
3. сервер → ответ, когда появились данные
4. клиент сразу делает новый запрос

Это лучше обычного polling, но всё равно сложнее и не такое удобное, как WebSocket или SSE.

## SSE — Server-Sent Events

- SSE — это механизм, при котором сервер держит HTTP-соединение открытым и отправляет клиенту события.

Главное: SSE — односторонний поток: сервер → клиент
...
Сервер может отправлять события в специальном формате:

- data: Новое уведомление
- data: Ещё одно уведомление

И браузер будет получать их без постоянных новых запросов.

### Где хорош SSE

SSE отлично подходит, когда серверу нужно пушить обновления клиенту, но клиенту не нужно постоянно отправлять данные обратно по тому же соединению.
...
- прогресс генерации отчёта;
- котировки;
- новости;
- статус доставки;
- обновление dashboard.

Например, пользователь нажал «Сформировать отчёт». Сервер долго считает, а фронт получает прогресс:
...
Для этого SSE прям очень приятен.

#### Плюсы SSE

- проще WebSocket;
- работает поверх обычного HTTP;
- браузер сам умеет переподключаться;
- хорошо подходит для server → client событий;
- можно использовать обычную авторизацию через cookies.

У EventSource есть автоматическое переподключение. Если соединение оборвалось, браузер попробует восстановить его.

#### Ограничения SSE

Главное ограничение: SSE — только сервер → клиент.

Если клиенту нужно отправить данные на сервер, он обычно делает обычный fetch.

Например:

- сервер → клиент: новое уведомление через SSE
- клиент → сервер: отметить как прочитанное через POST

Ещё нюанс: SSE передаёт в основном текстовые данные. Для бинарных данных WebSocket обычно удобнее.

## WebSocket

- WebSocket — это постоянное двустороннее соединение между клиентом и сервером.

Главное: WebSocket — двусторонний канал: **клиент ↔ сервер**

Сначала клиент делает HTTP-запрос с просьбой «обновить» соединение до WebSocket. После этого соединение остаётся открытым, и обе стороны могут отправлять сообщения в любой момент.
...
Здесь клиент может и получать, и отправлять сообщения по одному и тому же соединению.

#### Где хорош WebSocket

WebSocket нужен, когда важна активная двусторонняя коммуникация.
...
- доски вроде Miro/Figma;
- поддержка в реальном времени;
- системы, где клиент и сервер часто обмениваются событиями.

Например, в чате:

- клиент → сервер: пользователь отправил сообщение
- сервер → всем клиентам: новое сообщение
- клиент → сервер: пользователь печатает
- сервер → другому клиенту: "Настя печатает..."

Тут WebSocket подходит гораздо лучше, чем обычный HTTP.

## WebSocket vs SSE

Самое главное сравнение:

Критерий | SSE | WebSocket
Направление | Сервер → клиент | Клиент ↔ сервер
Протокол | HTTP | WebSocket-протокол после upgrade
Сложность | Проще |Сложнее
Автопереподключение | Есть встроенно | Нужно делать самому
Бинарные данные | Не очень удобно | Поддерживаются
Лучшие сценарии | Уведомления, прогресс, live-ленты | Чаты, игры, совместное редактирование
Отправка от клиента | Через обычный HTTP-запрос | Через тот же сокет

## Как выбрать

_Можно выбрать SSE_ eсли сервер просто сообщает клиенту новости:
...
_Лучше WebSocket_ если клиент и сервер активно обмениваются сообщениями:

- чат;
- игра;
- онлайн-доска;
- совместное редактирование;
- частые события в обе стороны;

Если обновления редкие и простые, иногда достаточно обычного polling. Не надо тащить WebSocket туда, где раз в минуту нужно проверить статус заказа. Это будет как арендовать фуру, чтобы перевезти один кактус.

## Почему не всегда WebSocket

WebSocket мощный, но у него есть цена:

- надо держать постоянные соединения;
- сложнее масштабировать;
- нужно думать про переподключение;
- нужно обрабатывать heartbeat/ping-pong;
- сложнее балансировка;
- сложнее отладка, чем обычный HTTP.

Если у тебя 10 пользователей — окей Если 100 тысяч одновременных подключений — уже нужна нормальная архитектура.

_Часто используют Redis Pub/Sub, message broker, sticky sessions или отдельный realtime-сервис._

## Что такое ws:// и wss://

Для WebSocket есть два варианта:

1. ws:// — обычное незашифрованное соединение
2. wss:// — защищённое соединение поверх TLS
...
В production обычно нужен wss://.

_Для обмена данными в реальном времени используют **polling, long polling, Server-Sent Events и WebSocket.** Polling — это периодические HTTP-запросы клиента к серверу, простой, но неэффективный способ. SSE позволяет серверу отправлять события клиенту по открытому HTTP-соединению и хорошо подходит для уведомлений, live-логов и прогресса операций. WebSocket создаёт постоянное двустороннее соединение, где и клиент, и сервер могут отправлять сообщения в любой момент. WebSocket лучше подходит для чатов, онлайн-игр и совместного редактирования. Критерий выбора простой: если нужен поток событий только от сервера к клиенту — SSE, если нужен активный обмен в обе стороны — WebSocket._

# Вопрос 1 Как сделать интерактивный элемент доступным для клавиатуры и скринридера

Главная идея такая: _доступный интерактивный элемент должен быть понятен не только глазами, но и клавиатуре, браузеру и вспомогательным технологиям._ То есть пользователь должен понимать:
...
Самое важное правило: Сначала использовать нативные HTML-элементы, а не делать всё из div.

Например, если элемент нажимается и выполняет действие — это кнопка:
...
Почему button лучше? Потому что браузер уже знает, что это интерактивный элемент. У него автоматически есть:

- фокус с клавиатуры;
- активация через Enter и Space;
- роль button для скринридера;
- disabled-состояние;
- участие в порядке табуляции.

А если сделать кнопку из div, то всё это придётся вручную восстанавливать. И легко что-нибудь забыть.

Ссылку тоже надо делать ссылкой:

<a href="/profile">Профиль</a>

Если элемент ведёт **на другую страницу** или другой URL — это a.

Если элемент выполняет **действие** на текущей странице — это button.
...
Пользователь должен иметь возможность пользоваться интерфейсом без мыши.

Обычно навигация такая:

1. Tab — перейти к следующему интерактивному элементу
2. Shift+Tab — перейти к предыдущему
3. Enter — активировать ссылку или кнопку
4. Space — активировать кнопку, checkbox
5. Esc — закрыть модалку, меню, popover
6. стрелки — навигация внутри сложных компонентов

Нативные элементы уже умеют большую часть этого.
...
Они автоматически доступны через Tab.

### Фокус

Фокус показывает, какой элемент сейчас активен для клавиатуры.
...
Если убрать outline и не дать замену, пользователь с клавиатуры просто потеряется: он нажимает Tab и не понимает, где находится. Это прям accessibility crime scene.

Если хочется кастомный стиль — окей:
...
focus-visible хорош тем, что обычно показывает фокус именно при клавиатурной навигации, а не при каждом клике мышкой.

## Порядок табуляции

Порядок перехода по Tab должен быть логичным и обычно совпадать с визуальным порядком страницы.

Лучше всего, когда порядок задаётся самим HTML:
...
Не стоит без нужды использовать положительный tabindex:

```html
<button tabindex="5">...</button> <button tabindex="1">...</button>
```

Так можно создать хаотичный порядок фокуса. Пользователь будет прыгать по странице как мячик в стиральной машине.

Нормальные значения:

1. tabindex="0"
   Добавляет элемент в обычный порядок табуляции.
2. tabindex="-1"
   Позволяет поставить фокус программно, но элемент не будет доступен через обычный Tab.

   Например, tabindex="-1" часто используют для заголовка после перехода или для модального окна:

   <h1 tabindex="-1">Настройки профиля</h1>

3. А вот tabindex="1", tabindex="2" и так далее лучше избегать.

Если всё-таки нужен кастомный элемент

Иногда дизайн требует кастомный компонент. Например, сделали раскрывающийся блок.

Плохо:

```html
<div class="accordion-title" onclick="toggle()">Подробнее</div>
```

Лучше, если это действие — использовать button:
...
- button — нормальная семантика;
- aria-expanded — говорит, раскрыт блок или нет;
- aria-controls — связывает кнопку с блоком, которым она управляет;
- hidden — скрывает контент, когда он не нужен.

При открытии нужно менять состояние:
...
<div id="details">Дополнительная информация</div>
```

Скринридер сможет сообщить пользователю, что кнопка раскрывает/сворачивает область.

## ARIA: когда нужна и когда не нужна

- ARIA — это набор атрибутов, которые помогают описать роль, имя и состояние элемента для вспомогательных технологий.

Но есть важное правило:

Лучше использовать нативный HTML, чем пытаться чинить плохую семантику через ARIA.
...
ARIA нужна, **когда нативного HTML не хватает**. Например:

<button aria-expanded="false">
    Открыть меню
</button>

Или когда у кнопки нет видимого текста, только иконка:

<button type="button" aria-label="Закрыть окно">
    ✕
</button>

_Без aria-label скринридер мог бы прочитать просто «крестик» или вообще что-то неинформативное._

## Accessible name

У интерактивного элемента должно быть понятное имя.

Хорошо:

<button>Удалить товар</button>

Плохо: _если из контекста непонятно, что именно произойдёт._

<button>OK</button>

Для иконок:

<button aria-label="Удалить товар">
    🗑
</button>

Для ссылок тоже важно:

<a href="/report.pdf">Скачать отчёт за июнь</a>

Плохо:

<a href="/report.pdf">Нажмите здесь</a>

Скринридеры могут отдельно читать список ссылок, и «нажмите здесь» вообще не помогает понять, куда ведёт ссылка.

## Модальные окна

Когда открывается модальное окно:

- фокус должен перейти внутрь модалки;
- Tab должен ходить только по элементам внутри модалки;
- Esc часто закрывает модалку;
- после закрытия фокус должен вернуться на кнопку, которая её открыла;
- фон не должен быть доступен скринридеру и клавиатуре.

Например:

```html
<div role="dialog" aria-modal="true" aria-labelledby="modal-title">
  <h2 id="modal-title">Удалить запись?</h2>
  <button>Отмена</button>
  <button>Удалить</button>
</div>
```

role="dialog" и aria-modal="true" помогают вспомогательным технологиям понять, что пользователь сейчас внутри модального окна.

_Чтобы интерактивный элемент был доступен, сначала нужно использовать семантический HTML: button для действий, a для переходов, нативные input, select, textarea для форм. Элемент должен быть доступен с клавиатуры через Tab, активироваться стандартными клавишами, иметь видимый фокус и логичный порядок табуляции. Не стоит убирать outline без замены и использовать положительные значения tabindex. Для скринридеров у элемента должны быть понятные роль, имя и состояние. ARIA используют, когда нативной семантики недостаточно, например aria-label, aria-expanded, aria-controls, aria-modal, но ARIA не должна заменять правильный HTML._

# Вопрос 2 Требования к доступной веб-форме

Форма доступна, когда пользователь понимает:
...
- где ошибка;
- как её исправить;
- какая подсказка относится к какому полю.

### Метка поля: label

Каждое поле ввода должно иметь связанную метку.
...
Теперь при клике на текст Email фокус перейдёт в поле. Скринридер тоже сможет прочитать: «Email, поле ввода».
...
Но вариант через for и id часто удобнее, особенно для сложной разметки.
...
Визуально вроде понятно, но для скринридера это может быть просто текст рядом с полем, а не настоящая метка.
...
- у него обычно слабый контраст;
- пользователь может забыть, что именно вводил;
- скринридеры не всегда воспринимают его как полноценную метку.
...
- То есть label — название поля, placeholder — пример формата. Не наоборот.

### Подсказка через aria-describedby

Если у поля есть пояснение, его нужно связать с полем.
...
Теперь скринридер при фокусе на поле сможет прочитать не только название, но и подсказку.
...
- видимой;
- понятной;
- связанной с конкретным полем;
- не только красной рамкой;
- доступной для скринридера.
...
и всё. Пользователь видит красную рамку, но не знает, что не так. А пользователь скринридера может вообще не понять, что поле ошибочное.
...
- aria-describedby="email-error" — связывает поле с текстом ошибки.

Если есть и подсказка, и ошибка, можно указать оба id:
...
### Сообщение об ошибке после отправки

Если пользователь отправил форму, а ошибки появились сверху, важно дать ему понять, что произошло.

Например:

```js
<div role="alert">Форма содержит ошибки. Проверьте email и пароль.</div>
```

- role="alert" может быть озвучен скринридером автоматически.

Но не надо злоупотреблять: если на каждое движение мыши всё будет орать, пользователь просто уйдёт жить в лес.

Можно также после неудачной отправки перевести фокус на блок ошибок:

```js
<div tabindex="-1" id="form-errors">
  <h2>Проверьте форму</h2>
  <ul>
    <li>
      <a href="#email">Введите корректный email</a>
    </li>
    <li>
      <a href="#password">Пароль слишком короткий</a>
    </li>
  </ul>
</div>
```

Так пользователь сразу понимает, что форма не отправилась и где проблемы.

## Обязательные поля

Можно использовать нативный **required**:

<label for="name">Имя</label>
<input id="name" name="name" required>

Но визуально тоже стоит обозначить:
...
> Если используешь только звёздочку без объяснения, часть пользователей может не понять.

Иногда добавляют aria-required="true", но для обычного HTML-поля required обычно достаточно, потому что оно уже понятно браузеру и вспомогательным технологиям.

Группы полей: fieldset и legend

Для radio и checkbox-групп нужен общий вопрос.
...
Визуально понятно, но связь группы может быть слабой.
...
legend сообщает скринридеру общий смысл группы.

То же самое для чекбоксов:
...
Тип поля помогает и доступности, и UX.
...
Например, на телефоне type="email" покажет клавиатуру с @, а браузер сможет сделать базовую проверку формата.

Также полезен autocomplete:
...
Это помогает автозаполнению, менеджерам паролей и пользователям, которым сложно вводить данные вручную.
...
- Зелёная рамка = всё хорошо

Почему плохо?

Пользователь может не различать цвета. Или скринридер не увидит цвет вообще.

Лучше:

красная рамка + текст ошибки + иконка + aria-invalid

Например:

<p id="email-error">
    Ошибка: введите email в формате name@example.com.
</p>
Кнопка отправки

Кнопка должна быть настоящей кнопкой:

<button type="submit">
    Зарегистрироваться
</button>

Если кнопка заблокирована, нужно быть осторожной.

<button type="submit" disabled>
    Зарегистрироваться
</button>

disabled-кнопка не попадает в фокус. Иногда это нормально, но иногда пользователь не понимает, почему он не может отправить форму.

Часто лучше оставить кнопку доступной и после нажатия показать понятные ошибки:

Заполните email.
Пароль должен быть не короче 8 символов.
Хороший пример доступного поля
...
Здесь есть всё важное:

- label связан с input;
- есть правильный type;
- есть autocomplete;
- есть required;
- подсказка связана через aria-describedby;
- ошибка связана через aria-describedby;
- ошибочное состояние обозначено через aria-invalid.

_В доступной форме каждое поле должно иметь связанную метку label, обычно через for и id. Placeholder не должен заменять label, он подходит только для примера ввода. Подсказки и сообщения об ошибках нужно связывать с полем через aria-describedby. Если поле содержит ошибку, можно использовать aria-invalid="true" и текстовое сообщение, а не только цветовую индикацию. Для групп radio и checkbox используют fieldset и legend. Также важно применять подходящие типы input, атрибуты required и autocomplete, логичный порядок фокуса и понятную кнопку отправки. Ошибки после отправки должны быть видимыми, понятными и доступными для скринридера._

Мини-проверка:

Почему это хуже для доступности?

<span>Email</span>
<input type="email" placeholder="Введите email">

И как лучше связать текст Email с полем?
