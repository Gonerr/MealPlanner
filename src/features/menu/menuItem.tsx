...
const MenuItem: React.FC<MenuItemProps> = ({ dish }) => {
    ...
    const isAdminMode = useSelector((state: RootState) =>
        selectIsAdminMode(state)
    );
    ...
    const handleSave = useCallback(async () => {
        try {
            await dispatch(
                updateRecipe({ id: dish.id, recipe: editForm })
            ).unwrap();
    ...
    const categoryLabels: Record<Dish["category"], React.ReactNode> = {
        salads: (
            <>
                <Salad className="me-1 h-3 w-3" /> Салаты
            </>
        ),
        soups: (
            <>
                <Soup className="me-1 h-3 w-3" /> Супы
            </>
        ),
        main: (
            <>
                <ForkKnife className="me-1 h-3 w-3" /> Основное
            </>
        ),
        desserts: (
            <>
                <CakeSlice className="me-1 h-3 w-3" />
                Десерты
            </>
        ),
        snacks: (
            <>
                <Popcorn className="me-1 h-3 w-3" />
                Закуски
            </>
        ),
        drinks: (
            <>
                <GlassWater className="me-1 h-3 w-3" />
                Напитки
            </>
        ),
        specials: (
