...
export const POST = withAuthHandler(async (request, { db, user }) => {
  const safeDb = new safeDB(db);
  const { name, slug, color, description, icon, display_order } =
    await request.json();
  ...
  const existing = await safeDb.get("SELECT * FROM categories WHERE slug = ?", [
    slug,
  ]);
...
export const PUT = withAuthHandler(async (request, { db, params }) => {
  const safeDb = new safeDB(db);
  const { name, slug, description, color, icon, display_order, is_active } =
    await request.json();
  ...
  // Проверяем уникальность slug
  const existing = await safeDb.get("SELECT * FROM categories WHERE id = ?", [
    id,
  ]);
